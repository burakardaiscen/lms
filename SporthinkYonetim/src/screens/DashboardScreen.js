  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center bg-slate-50">
        <ActivityIndicator size="large" color={SPORTHINK_RED} />
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {/* ÜST PROFİL ALANI - Sporthink Kırmızı Teması */}
        <View className="bg-red-600 rounded-b-3xl p-6 shadow-md pb-8">
          <View className="flex-row justify-between items-center mb-6">
            <View>
              <Text className="text-red-100 text-sm font-bold mb-1">Tekrar Hoş Geldin,</Text>
              <Text className="text-white text-2xl font-extrabold">{user?.name}</Text>
              <Text className="text-red-200 text-xs font-medium mt-1">{user?.department} • {user?.role}</Text>
            </View>
            ...
            >
              ...
              <View className="absolute top-2 right-2 w-3 h-3 bg-amber-400 rounded-full border-2 border-red-600" />
            </TouchableOpacity>
          </View>
          ...
          <View className="bg-white/10 rounded-2xl p-4 flex-row items-center justify-between border border-white/20">
            <View className="flex-row items-center flex-1">
              <View className="bg-white w-14 h-14 rounded-full items-center justify-center border-4 border-white/20 shadow-sm">
                <Text className="text-red-700 font-black text-xl">{userLevel}</Text>
              </View>
              <View className="ml-4 flex-1 mr-3">
                ...
                <View className="w-full bg-white/20 h-2 rounded-full mt-1.5 overflow-hidden">
                  <View className="bg-white h-full w-2/3 rounded-full" />
                </View>
                <Text className="text-red-100 text-[10px] mt-1.5 font-bold uppercase tracking-wider">{stats?.xp} Toplam XP</Text>
              </View>
            </View>
            
            {/* MAĞAZAYA GİDEN BUTON */}
            <TouchableOpacity 
              onPress={() => navigation.navigate('RewardStore')}
              activeOpacity={0.8}
              className="bg-white/20 border border-white/30 px-4 py-3 rounded-2xl items-center justify-center"
            >
              <View className="flex-row items-center mb-1">
                <MaterialCommunityIcons name="database" size={16} color="white" />
                <Text className="text-white font-black ml-1.5 text-base">{stats?.coin || 0}</Text>
              </View>
              <Text className="text-red-100 text-[9px] font-bold uppercase">Mağaza</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View className="p-5 -mt-4">
          ...
          <View className="flex-row justify-between mb-6">
            ...
            >
              <View className="bg-red-50 p-3 rounded-full mb-3">
                <Ionicons name="play-circle" size={28} color={SPORTHINK_RED} />
              </View>
          ...
          <View className="flex-row flex-wrap justify-between">
            <View className="bg-white w-[48%] p-5 rounded-3xl mb-4 border border-slate-100 shadow-sm">
              <View className="bg-red-50 w-10 h-10 rounded-full items-center justify-center mb-3">
                <Ionicons name="trending-up" size={20} color={SPORTHINK_RED} />
              </View>
          ...
          <View className={`p-5 rounded-3xl flex-row items-center mt-2 shadow-sm border ${isAllCompleted ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
            <View className={`${isAllCompleted ? 'bg-emerald-100' : 'bg-red-100'} p-3 rounded-full mr-4`}>
              <Ionicons 
                name={isAllCompleted ? "checkmark-done" : "alert-circle"} 
                size={28} 
                color={isAllCompleted ? "#059669" : SPORTHINK_RED} 
              />
            </View>
            <View className="flex-1">
              <Text className={`font-black text-base mb-1 ${isAllCompleted ? 'text-emerald-800' : 'text-red-800'}`}>
                {isAllCompleted ? 'Harika İş Çıkardın!' : 'Eğitim Zamanı'}
              </Text>
              <Text className={`font-medium text-xs leading-5 ${isAllCompleted ? 'text-emerald-700' : 'text-red-700'}`}>
                {stats?.kritikUyari}
              </Text>
            </View>
          </View>
